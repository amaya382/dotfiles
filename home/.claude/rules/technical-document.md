...
description: Language-independent rules for technical prose. Applies to any text whose job is to convey technical content — specifications, design docs, ADRs, RFCs, README and other repository documentation, pull request descriptions, git commit messages, code comments and docstrings, technical blog posts and explainers, postmortems, issue and ticket writeups, design review and meeting notes. Covers authorial presence, false agency, paragraph structure, information weighting, hierarchy and placement (overview-first ordering, heading levels, the demotion ladder, list structure, choosing between prose, list, table, and figure), logical rigor, reader load, restraint in rhetoric, structural AI tells, redundancy elimination, headings, honesty toward readers, standards for examples and code samples, and figure quality and notation. Short forms take only the sections that have room to apply. Pair with a language-specific ruleset (technical-document-ja.md, technical-document-en.md) for vocabulary, punctuation, and idiom-level checks. / 技術文章の言語非依存な文章規範。技術的な内容を伝える文章すべてが対象——仕様書、設計書、ADR、RFC、README などのリポジトリ文書、Pull Request の description、git commit メッセージ、コードコメントや docstring、技術ブログ記事や解説記事、ポストモーテム、Issue やチケットの記述、設計レビューや議事録。主体と語り、false agency、段落構成、情報の重み付け、階層と配置（overview 先出し、見出し階層、降格ラダー、リスト構造、prose/list/table/figure の使い分け）、論証の厳密さ、読者負荷、演出の抑制、構造レベルの AI 臭、冗長の排除、見出し、読者への誠実さ、例とコードの規範、図の品質と表記を定める。短い形式では適用できる節だけを使う。語彙・句読点・慣用句レベルは言語別規範（technical-document-ja.md、technical-document-en.md）と併用する。
---
